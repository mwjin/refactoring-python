...
def test_days_since_last_cleaning():
    ...
    assert (
        Scroll(0, last_cleaned_date, 1, catalog).days_since_last_cleaning(
            target_date
        )
        == expect
    )
...
def test_needs_cleaning():
    ...
    catalog = {1: CatalogItem(1, "Catalog1", [])}
    assert not Scroll(
        1, target_date - timedelta(days=1500), 1, catalog
    ).needs_cleaning(target_date)
    assert Scroll(
        2, target_date - timedelta(days=1501), 1, catalog
    ).needs_cleaning(target_date)
...
def test_needs_cleaning_with_revered_tag():
    ...
    assert not Scroll(
        1, target_date - timedelta(days=700), 1, catalog
    ).needs_cleaning(target_date)
    assert Scroll(
        2, target_date - timedelta(days=701), 1, catalog
    ).needs_cleaning(target_date)
